<?php 
    /** @var $pdo\PDO */
    require_once "../data/database.php";
    $errors=[];
    $user_id="";

    //check server method
    if ($_SERVER["REQUEST_METHOD"] === "POST"){

        $firstname=$_POST['inputFirstname'];
        $lastname=$_POST['inputLastname'];
        $email=$_POST['inputEmail'];
        $user_id=$_POST['inputUserID'];
        $password=$_POST['inputPassword'];
        $gender=$_POST['gender'];

        //password check
        $uppercase = preg_match('@[A-Z]@', $password);
        $lowercase = preg_match('@[a-z]@', $password);
        $number    = preg_match('@[0-9]@', $password);
        $specialChars = preg_match('@[^\w]@', $password);

        //check if userID is unique
        $statementID=$conn->prepare('SELECT user_id
                                    FROM student_user
                                    WHERE user_id = :username'
                                    );
        $statementID->bindValue(':username',$user_id);
        $statementID->execute();

        $validID = $statementID->fetchAll(PDO::FETCH_ASSOC);

        // check if all the boxes are filled
        
        if(!$user_id || !$firstname || !$lastname || !$email || !$password )
        {
            
            $errors[]='Please fill in all the room';
        }

        if(!empty($validID)){
            $errors[]='This ID is already taken.';
        }
        else if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
            $errors[]= 'Password should be at least 8 characters in length and should include 
             at least one upper case letter, one number, and one special character.';
         }
    
        if(empty($errors)){
            $statement2=$conn->prepare("INSERT INTO 
                                        student_user(user_id,password)
                                        VALUES
                                        (:id,:pass)");
            $statement2->bindValue(':id',$user_id);
            $statement2->bindValue(':pass',$password);
            $statement2->execute();
    
            $statement=$conn->prepare("INSERT INTO 
                                    student (firstname,lastname,student_id,user_id,email,gender) 
                                    VALUES 
                                    (:firstname,:lastname,:student_id,:user_id,:email,:gender)");
                                
            $statement->bindValue(':firstname',$firstname);
            $statement->bindValue(':lastname',$lastname);
            $statement->bindValue(':student_id',$user_id);
            $statement->bindValue(':user_id',$user_id);
            $statement->bindValue(':email',$email);
            $statement->bindValue(':gender',$gender);
            $statement->execute();
    
            //go back to login page
            header("Location:../login/login.php");
        }
    }
    ?>


<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel = "icon" href ="../assets/icons/logo.png" type = "image/x-icon">
    <link rel="stylesheet" href="	https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="signup.css">
    <title>Sign Up</title>

</head>
<body>
    <div class='container border border-primary'>
        
        <form class="row g-3" action="signup_student.php" method="POST">
            <div class="col-md-6">
                <label for="inputFirstname" class="form-label">First Name</label>
                <input type="text" class="form-control" name="inputFirstname">
            </div>
            <div class="col-md-6">
                <label for="inputLastname" class="form-label">Last Name</label>
                <input type="text" class="form-control" name="inputLastname" >
            </div>
            <div class="col-12">
                <label for="inputusername" class="form-label">User ID</label>
                <input type="text" class="form-control" name="inputUserID" placeholder="Please enter your student ID no" >
            </div>
            <div class="col-12">
                <label for="inputEmail4" class="form-label">Email</label>
                <input type="email" class="form-control" name="inputEmail" >
            </div>
            <div class="col-12">
                <label for="inputEmail4" class="form-label">Password</label>
                <input type="password" class="form-control" name="inputPassword" >
            </div>

        
            <div class="col-md-6">
                <label for="inputCity" class="form-label">City</label>
                <input type="text" class="form-control" name="inputCity" >
            </div>

            <div class="dropdown ">
                <label for="" class="form-label">Gender</label><br>
                <select name="gender" class="form-select"> 
                <option selected>Choose...</option>
                <option value="">Male</option>
                <option value="">Female</option>
                <option value="">Other</option>
                </select>
            </div>

            <?php if(!empty($errors)): ?>
            <div class="alert alert-danger">
                <?php foreach($errors as $error): ?>
                    <div ><?php echo $error ?></div>
                <?php endforeach; ?>
            </div>    
            <?php endif; ?>
            
            <div class="btn-group">
                <input type="submit" class="btn btn-primary" value="Create New Account">
            </div>
        </form> 
        <p class="link">Have an account? <a href="../login/login.php">Sign In</a></p>
    </div>
    


    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/js/bootstrap.bundle.min.js"></script>


    <script>
    if ( window.history.replaceState ) {
        window.history.replaceState( null, null, window.location.href );
    }
    </script>
</body>
</html>